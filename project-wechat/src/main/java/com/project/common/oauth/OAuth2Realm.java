...
public class OAuth2Realm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        Long dealUserId = redisUtils.get(RedisKeys.Wx.WxLoginToken(accessToken), Long.class);
        //token失效
        if (dealUserId == null) {
            throw new IncorrectCredentialsException("登录过期，请重新登录");
        }
        DealUserLoginVo dealUserLoginVo = redisUtils.get(RedisKeys.Wx.WxLogin(new StringBuilder().append(RedisKeyConstant.WX_LOGIN).append("_").append(dealUserId).toString()), DealUserLoginVo.class);
